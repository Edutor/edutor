...
fun urlSolution(row: ResultRow) = Url.Solution(
        row[SOLUTIONS.id],
        user(row),
        urlChallenge(row),
        row[URL_SOLUTIONS.url]
)
...
fun SOLUTIONS.persist(solution: Solution): Solution {
    if (solution.id == 0) return transaction {
        ...
        when (solution) {
            ...
            is Url.Solution -> URL_SOLUTIONS.insert {
                it[id] = solutionId
                it[url] = solution.url
            }
        ...
        when (solution) {
            ...
            is Url.Solution -> URL_SOLUTIONS.update({ id eq solution.id }) {
                it[url] = solution.url
            }
        }
        solution
